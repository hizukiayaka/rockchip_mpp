...
static MPP_RET h264e_check_mpp_cfg(MppEncConfig *mpp_cfg)
{
    ...
    if (i >= count) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid profile %d set to default baseline\n", mpp_cfg->profile);
    ...
    if (i >= count) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid level %d set to default 4.0\n", mpp_cfg->level);
        mpp_cfg->level = H264_LEVEL_3;
    }

    if (mpp_cfg->fps_in <= 0) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid input fps %d will be set to default 30\n", mpp_cfg->fps_in);
        mpp_cfg->fps_in = 30;
    }

    if (mpp_cfg->fps_out <= 0) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid output fps %d will be set to fps_in 30\n", mpp_cfg->fps_out, mpp_cfg->fps_in);
        mpp_cfg->fps_out = mpp_cfg->fps_in;
    }

    if (mpp_cfg->gop <= 0) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid gop %d will be set to fps_out 30\n", mpp_cfg->gop, mpp_cfg->fps_out);
        mpp_cfg->gop = mpp_cfg->fps_out;
    }
    ...
    if (mpp_cfg->hor_stride <= 0) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid hor_stride %d will be set to %d\n", mpp_cfg->hor_stride, MPP_ALIGN(mpp_cfg->width, 16));
        mpp_cfg->hor_stride = MPP_ALIGN(mpp_cfg->width, 16);
    }

    if (mpp_cfg->ver_stride <= 0) {
        mpp_dbg_f(MPP_DBG_NORMAL, "invalid ver_stride %d will be set to %d\n", mpp_cfg->ver_stride, MPP_ALIGN(mpp_cfg->height, 16));
        mpp_cfg->ver_stride = MPP_ALIGN(mpp_cfg->height, 16);
    }
...
MPP_RET h264e_config(void *ctx, RK_S32 cmd, void *param)
{
    ...
    case SET_ENC_RC_CFG : {
        ...
        if (ret) {
            ...
            mpp_dbg_f(MPP_DBG_NORMAL, "Get rate control: qp %2d [%2d, %2d] bps %8d\n",
                      oriRcCfg.qpHdr, oriRcCfg.qpMin, oriRcCfg.qpMax, oriRcCfg.bitPerSecond);

            mpp_dbg_f(MPP_DBG_NORMAL, "pic %d mb %d skip %d hrd %d cpbSize %d gopLen %d\n",
            ...
            mpp_dbg_f(MPP_DBG_NORMAL, "Set rate control: qp %2d [%2d, %2d] bps %8d\n",
                    oriRcCfg.qpHdr, oriRcCfg.qpMin, oriRcCfg.qpMax, oriRcCfg.bitPerSecond);
