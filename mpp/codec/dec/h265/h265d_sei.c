...
static RK_S32 mastering_display_colour_volume(HEVCContext *s)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        h265d_dbg(H265D_DBG_SEI, "display_primaries_x[%d] = %d", i, value);
        READ_BITS(gb, 16, &value);
        h265d_dbg(H265D_DBG_SEI, "display_primaries_y[%d] = %d", i, value);
    }
    READ_BITS(gb, 16, &value);
    h265d_dbg(H265D_DBG_SEI, "white_point_x = %d", value);
    READ_BITS(gb, 16, &value);
    h265d_dbg(H265D_DBG_SEI, "white_point_y = %d", value);
    mpp_read_longbits(gb, 32, &lum);
    h265d_dbg(H265D_DBG_SEI, "max_display_mastering_lum = %d", lum);
    mpp_read_longbits(gb, 32, &lum);
    h265d_dbg(H265D_DBG_SEI, "min_display_mastering_lum = %d", lum);
