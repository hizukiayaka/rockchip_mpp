...
static void hal_h264e_vpu_dump_mpp_reg_in(h264e_hal_context *ctx)
{
    ...
    if (fp) {
        ...
        fprintf(fp, "#FRAME %d:\n", ctx->frame_cnt);
        for (k = 0; k < VEPU_H264E_VEPU1_NUM_REGS; k++) {
...
static MPP_RET
hal_h264e_vpu_free_buffers(h264e_hal_context *ctx)
{
    RK_S32 k = 0;
...
static void
hal_h264e_vpu_write_cabac_table(h264e_syntax *syn, MppBuffer hw_cabac_tab_buf)
{
...
static MPP_RET
hal_h264e_vpu_allocate_buffers(h264e_hal_context *ctx, h264e_syntax *syn)
{
    MPP_RET ret = MPP_OK;
    RK_S32 k = 0;
    h264e_hal_vpu_buffers *buffers = (h264e_hal_vpu_buffers *)ctx->buffers;
    ...
    ret = mpp_buffer_get(buffers->hw_buf_grp, &buffers->hw_nal_size_table_buf,
		    (sizeof(RK_U32) * (syn->pic_luma_height + 1) + 7) & (~7));
    if (ret) {
...
static MPP_RET
hal_h264e_vpu_stream_buffer_status(h264e_hal_vpu_stream *stream)
{
...
static MPP_RET
hal_h264e_vpu_stream_buffer_reset(h264e_hal_vpu_stream *strmbuf)
{
...
static MPP_RET
hal_h264e_vpu_stream_buffer_init(h264e_hal_vpu_stream *strmbuf, RK_S32 size)
{
...
static void
hal_h264e_vpu_stream_put_bits(h264e_hal_vpu_stream *buffer,
...
static void hal_h264e_vpu_stream_put_bits_with_detect
...
static void hal_h264e_vpu_rbsp_trailing_bits(h264e_hal_vpu_stream * stream)
{
...
static void hal_h264e_vpu_write_se
(h264e_hal_vpu_stream *fifo, RK_S32 val, const char *name)
{
    RK_U32 tmp;
...
static MPP_RET hal_h264e_vpu_nal_start
(h264e_hal_vpu_stream * stream, RK_S32 nalRefIdc, rkvenc_nal_unit_type nalUnitType)
{
...
static MPP_RET hal_h264e_vpu_write_sps
(h264e_hal_vpu_stream *stream, h264e_hal_sps *sps)
{
    h264e_hal_debug_enter();
...
static MPP_RET hal_h264e_vpu_write_pps
(h264e_hal_vpu_stream *stream, h264e_hal_pps *pps)
{
    h264e_hal_debug_enter();
...
static void hal_h264e_vpu_set_sps
(h264e_hal_sps *sps, h264e_control_extra_info_cfg *cfg)
{
...
static void hal_h264e_vpu_set_pps
(h264e_hal_pps *pps, h264e_control_extra_info_cfg *cfg)
{
    pps->i_id = 0;
    pps->i_sps_id = 0;
...
MPP_RET hal_h264e_vepu1_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    val = VEPU_REG_MAD_QP_ADJUSTMENT (syn->mad_qp_delta)
          | VEPU_REG_MAD_THRESHOLD(syn->mad_threshold);
    ...
    val |= VEPU_REG_H264_SLICE_SIZE(syn->slice_size_mb_rows)
          | VEPU_REG_INTRA16X16_MODE(h264_intra16_favor[syn->qp]);
    ...
    val = VEPU_REG_IN_IMG_CHROMA_OFFSET(0)
          ...
          | VEPU_REG_IN_IMG_CTRL_FMT(hal_h264e_vpu_convert_csp
    ...
    val = VEPU_REG_RGB2YUV_CONVERSION_COEFF(syn->color_conversion_coeff_f)
          | VEPU_REG_RGB_MASK_B_MSB(syn->color_conversion_b_mask_msb)
...
MPP_RET hal_h264e_vepu1_start(void *hal, HalTaskInfo *task)
{
    ...
    if (ctx->vpu_socket > 0) {
        ...
        h264e_hal_log_detail("vpu client is sending %d regs", VEPU_H264E_VEPU1_NUM_REGS);
        if (MPP_OK != VPUClientSendReg(ctx->vpu_socket, p_regs, VEPU_H264E_VEPU1_NUM_REGS)) {
