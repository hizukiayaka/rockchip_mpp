...
#define MODULE_TAG "hal_h264d_vdpu1_pkt"
...
static RK_U32 check_dpb_buffer_is_valid_vpu1(H264dHalCtx_t *p_hal, RK_U32 dpb_idx)
{
    (void)p_hal;
    (void)dpb_idx;
    ...
    if (pp->field_pic_flag) {
        ...
        for (i = 0; i < 32; i++) {
            if (pp->RefFrameList[i / 2].bPicEntry == 0xff) { //!< invalid
                ...
                validTmp = check_dpb_buffer_is_valid_vpu1(p_hal, pp->RefFrameList[i / 2].Index7Bits);
        ...
        for (i = 0; i < 16; i++) {
            if (pp->RefFrameList[i].bPicEntry == 0xff) {  //!< invalid
                ...
                longTermflags = (longTermflags << 1) | longTermTmp;
                validTmp = check_dpb_buffer_is_valid_vpu1(p_hal, pp->RefFrameList[i].Index7Bits);
