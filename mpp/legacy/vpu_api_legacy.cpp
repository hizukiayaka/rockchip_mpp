...
RK_S32 VpuApiLegacy::init(VpuCodecContext *ctx, RK_U8 *extraData, RK_U32 extra_size)
{
    ...
    if (MPP_CTX_ENC == type) {
        ...
        mpp_dbg_f(MPP_DBG_NORMAL, "setup encoder rate control config:\n");
        mpp_dbg_f(MPP_DBG_NORMAL, "width %4d height %4d format %d\n", param->width, param->height, param->format);
        mpp_dbg_f(MPP_DBG_NORMAL, "rc_mode %s qp %d bps %d\n", (param->rc_mode) ? ("VBR") : ("CQP"), param->qp, param->bitRate);
        mpp_dbg_f(MPP_DBG_NORMAL, "fps in %d fps out %d gop %d\n", param->framerate, param->framerateout, param->intraPicRate);
        mpp_dbg_f(MPP_DBG_NORMAL, "setup encoder stream feature config:\n");
        mpp_dbg_f(MPP_DBG_NORMAL, "profile %d level %d cabac %d\n", param->profileIdc, param->levelIdc, param->enableCabac);
...
RK_S32 VpuApiLegacy:: decode_getoutframe(DecoderOut_t *aDecOut)
{
    ...
    if (ret || NULL == mframe) {
        ...
        if (buf) {
            ...
            vframe->vpumem.index = info.index;
...
RK_S32 VpuApiLegacy::control(VpuCodecContext *ctx, VPU_API_CMD cmd, void *param)
{
    ...
    case VPU_API_ENC_SETCFG : {
        ...
        if (mpi != NULL)
            ret = mpi->control(mpp_ctx, MPP_ENC_SET_CFG, (MppParam)&mpp_cfg);
    ...
    case VPU_API_ENC_GETCFG : {
        ...
        MPP_RET ret = MPP_NOK;
        if (mpi != NULL)
            ret = mpi->control(mpp_ctx, MPP_ENC_GET_CFG, (MppParam)&mpp_cfg);
        else
            return ret;
    ...
    RK_S32 ret = -1;
    if (mpicmd < MPI_CMD_BUTT && mpi != NULL)
