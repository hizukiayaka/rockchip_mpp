...
public:
        ...
        rkvpu_close_cxt(NULL)
    {
        ...
        if (rkapi_hdl) {
            ...
            vpu_api_dbg_func("dlopen vpu lib success\n");
        } else {
            vpu_api_dbg_func("dlopen vpu lib failed\n");
        }
    }
    ...
    ~VpulibDlsym() {
        if (rkapi_hdl) {
            dlclose(rkapi_hdl);
            vpu_api_dbg_func("dlclose vpu lib");
        }
    }
};
...
RK_S32 vpu_close_context(VpuCodecContext **ctx)
{
    ...
    if (s) {
        if (s->extra_cfg.reserved[0]) {
            ...
            vpu_api_dbg_func("org vpu_close_context ok");
        } else {
            if (s->flush)
                s->flush(s);
